...
class TestAnalysis(unittest.TestCase):
    ...
    def test_count_by_month(self):
        ...
        count = relative_age.analysis.count_by_month(df['Date of Birth'])
    ...
    def test_frequency_by_month(self):
        ...
        freq = relative_age.analysis.daily_freq_by_month(df['Date of Birth'])
        self.assertAlmostEqual(freq[3], 24 / 31. )

    def test_date_distance(self):

        self.assertEquals(relative_age.analysis.date_dist(date(2016, 1, 1), 1), 0)
        self.assertEquals(relative_age.analysis.date_dist(date(2016, 12, 31), 1), 365)

    def test_average_date_distance(self):
        df = self.load_test_file()

        average = relative_age.analysis.average_date_dist(df['Date of Birth'], 1)
        self.assertAlmostEqual(average, 164.225130890)

        average = relative_age.analysis.average_date_dist(df['Date of Birth'], 9)
        self.assertAlmostEqual(average, 175.314136126)

    def test_offset(self):
        df = self.load_test_file()

        average = relative_age.analysis.average_offset(df['Date of Birth'], 9)
        self.assertAlmostEqual(average, -7.310863874)

    def test_year_split(self):
        df = self.load_test_file()

        early, late = relative_age.analysis.year_split(df['Date of Birth'], 1)
    ...
    def test_year_split_percentage(self):
        ...
        percent = relative_age.analysis.year_split_percentage(df['Date of Birth'], 1)
    ...
    def test_chi_squared_exact(self):
        s = pd.Series(reduce(lambda x,y: x+y,[[date(2016,k,1)] * int(v * 4)
                                              for k,v in relative_age.analysis.month_days.to_dict().items()]))
        self.assertAlmostEqual(relative_age.analysis.chi_squared(s), 1.0)

    def test_chi_squared_all_jan(self):
        s = pd.Series([date(2015,1,1)] * 100)
        self.assertAlmostEqual(relative_age.analysis.chi_squared(s), 0.0)

    def test_group_by_period(self):

        s = pd.Series([date(2015,8,31), date(2015,9,1)])

        x = relative_age.analysis.group_by_period(s, 1, 4)
        self.assertTrue(pd.Series([0,0,2,0]).equals(x))

        x = relative_age.analysis.group_by_period(s, 9, 4)
